...
def _patch_write2_from_preset(
    body: str, preset_data: Dict[str, Any]
) -> Tuple[str, bool]:
    """
...
def _patch_eo7_mov_write(
    ...
    if tt == "colorspace":
        ocio_value = (
            preset_data.get("write_out_colorspace", "ACES - ACEScg")
            or "ACES - ACEScg"
        ).strip()
    else:
...
def _generate_nk_minimal(
    ...
    delivery_fmt = (
        preset_data.get("delivery_format", "EXR 16bit") or "EXR 16bit"
    ).upper()
    ...
    datatype_val = _preset_datatype_string(preset_data)
    comp_raw = (
        preset_data.get("write_compression", "PIZ Wavelet (32 scanlines)") or ""
    )
    comp_map = {
        "none": "none",
    ...
    metadata_raw = (
        preset_data.get("write_metadata", "all metadata") or "all metadata"
    )
...
def generate_nk_content(
    ...
    body = body.replace(
        _TEMPLATE_SAMPLE_SHOT_ROOT.replace("/", "\\"), shot_root_norm
    )
