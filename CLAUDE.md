...
- CD: `.github/workflows/release.yml` — `v*` 태그 push 시 gate(lint+test) 통과 → PyInstaller 빌드 → GitHub Release 업로드. gate 실패 시 빌드 안 함
...
## Windows 작업 표시줄 (AppUserModelID)

- `bpe.__main__.main()`이 `run_app` import **이전**에 `apply_explicit_app_user_model_id()`를 호출한다(PySide6 로드 전). `run_app` 안에서는 `QApplication` 생성 **직후** 한 번 더 호출한다(Qt 초기화가 프로세스 AUMID를 덮어쓸 수 있음). 추가로 `MainWindow`·`SplashScreen`은 네이티브 `winId()`가 생긴 뒤 `apply_app_user_model_id_to_hwnd`로 `SHGetPropertyStoreForWindow` + `PKEY_AppUserModel_ID`에 동일 문자열을 쓴다(실패 시 로그만). 문자열은 `BPE_APP_USER_MODEL_ID`(`Beluca.BPE`). **릴리즈 버전 문자열을 AUMID에 넣지 말 것** — 바꾸면 고정 핀과 실행 중 아이콘이 작업 표시줄에서 어긋날 수 있다.
- **수동 확인:** 빌드된 `BPE.exe` 실행 → 작업 표시줄 아이콘 우클릭 → 「작업 표시줄에 고정」 표시 여부. 공용 경로(예: `W:\...\BPE.exe`)에서 동일 경로·파일명으로 덮어써도 핀의 `.lnk`는 보통 유효하다(드라이브 매핑·경로 변경 시에만 깨짐).
